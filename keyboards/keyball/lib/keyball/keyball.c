...
report_mouse_t pointing_device_task_kb(report_mouse_t mouse_report) {
    report_mouse_t output = {0};
    motion_to_mouse(&mouse_report, &output, false, keyball.scroll_mode);
    // store mouse report for OLED.
    keyball.last_mouse = output;
    return pointing_device_task_user(output);
}
#endif
...
#else
    pointing_device_set_cpi(keyball.cpi_value);
#endif
...
void keyboard_post_init_kb(void) {
    ...
    // Assume ball is present - will be detected via split comms
